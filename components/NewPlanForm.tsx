  ...
  return (
    <Form {...form}>
      <form className="space-y-4">
        <h2 className="text-2xl font-bold">Create Travel Plan</h2>
        <FormField
          control={form.control}
          name="placeName"
          render={({field}) => (
            <FormItem>
              <FormLabel className="text-md font-[550]">Search for your destination city</FormLabel>
              <FormControl>
                <PlacesAutoComplete
                  field={field}
                  form={form}
        ...
        <FormField
          ...
          render={({field}) => (
            <FormItem className="flex flex-col">
              <FormLabel className="text-md font-[550]">Select Dates</FormLabel>
              <DateRangeSelector
                value={field.value}
        ...
        <FormField
          ...
          render={({field}) => (
            <FormItem>
              <FormLabel className="text-md font-[550] my-2">
                Select the kind of activities you want to do
                <span className="font-medium ml-1 text-[#6D5FFD]">(Optional)</span>
              </FormLabel>
              <FormControl>
                <div className="flex gap-2 flex-wrap">
                  {ACTIVITY_PREFERENCES.map((activity) => (
                    <label
                      ...
                      has-[:checked]:bg-[#6D5FFD60] has-[:checked]:opacity-100 dark:has-[:checked]:opacity-100
        ...
        <div className="w-full flex justify-between gap-1">
          <Button
            ...
            className="bg-[#6D5FFD] text-white hover:bg-black w-full"
          >
            {pendingEmptyPlan ? (
