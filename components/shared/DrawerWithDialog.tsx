    ...
    return (
      <>
        <Button
          ...
          className={`${shouldOpenForCreatePlan && "bg-[#6D5FFD] rounded-full hover:bg-black hover:text-white text-white flex gap-1"
            }`}
          onClick={() => {
            setOpen(true);
          }}
    ...
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerTrigger asChild>
        <Button
          ...
          className={`${shouldOpenForCreatePlan && "bg-[#6D5FFD] rounded-full text-white hover:text-white hover:bg-black flex gap-1"
            }`}
        >
          {shouldOpenForCreatePlan && <Backpack className="h-4 w-4" />}
          <span>{btnText}</span>
  ...
  return (
    <div>
      ...
      {/* <Link
        ...
        href={`${process.env.NEXT_PUBLIC_RAZORPAY_PAYMENT_PAGE_URL}${email ? `/?email=${email} ` : ``
          }`}
      >
