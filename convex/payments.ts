...
export const fulfill = internalMutation({
  ...
  handler: async (ctx, { stripeId, status }) => {
    ...
    const { _id, userId } = (await db
      .query("payments")
      .withIndex("by_paymentId", (q) => q.eq("paymentId", stripeId))
      .unique())!;

    if (!_id) {
      console.log(
        "error finding the payment id to update the status after fullfilling the payemnt"
      );
      return null;
    }
    ...
    await updateUserCredits(ctx, "", 0, userId);
  },
});
